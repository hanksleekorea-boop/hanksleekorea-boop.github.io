...
export function createSafeCircle({ ownerId, name = "My circle", maxMembers = 8 } = {}) {
  ...
  const challenges = [];
  const preferences = new Map([[owner, { encouragementNotifications: false }]]);
  ...
  return Object.freeze({
    ...
    setEncouragementNotifications(actorId, enabled) { if (!members.has(actorId)) return { ok: false, status: "forbidden" }; preferences.set(actorId, { encouragementNotifications: enabled === true }); return { ok: true, status: enabled === true ? "enabled" : "disabled" }; },
    encourage(actorId, targetId, kind = "with-you") { const allowed = new Set(["with-you", "take-your-time", "rest-is-ok"]); if (!members.has(actorId) || !members.has(targetId) || actorId === targetId || !allowed.has(kind)) return { ok: false, status: "invalid" }; const item = { id: `encouragement-${encouragements.length + 1}`, actorId, targetId, kind, pressureScore: 0, createdAt: new Date().toISOString() }; encouragements.push(item); return { ok: true, encouragement: clone(item), notificationSent: preferences.get(targetId)?.encouragementNotifications === true }; },
    createChallenge(actorId, input = {}) { if (!members.has(actorId)) return { ok: false, status: "forbidden" }; const title = String(input.title || "").trim().slice(0, 80); const actionTitle = String(input.actionTitle || "").trim().slice(0, 120); const days = Math.max(1, Math.min(30, Number(input.days) || 7)); if (!title || !actionTitle) return { ok: false, status: "invalid" }; const item = { id: `challenge-${challenges.length + 1}`, title, days, ranking: "disabled", participants: [{ ownerId: actorId, actionTitle }], createdAt: new Date().toISOString() }; challenges.push(item); return { ok: true, challenge: clone(item) }; },
    joinChallenge(actorId, challengeId, actionTitle) { if (!members.has(actorId)) return { ok: false, status: "forbidden" }; const challenge = challenges.find((item) => item.id === challengeId); const action = String(actionTitle || "").trim().slice(0, 120); if (!challenge || !action) return { ok: false, status: "invalid" }; challenge.participants = challenge.participants.filter((item) => item.ownerId !== actorId); challenge.participants.push({ ownerId: actorId, actionTitle: action }); return { ok: true, status: "joined", sameActionRequired: false }; },
    ...
    snapshot(viewerId) { if (!members.has(viewerId) || closed) return { ok: false, status: "forbidden" }; return { ok: true, circle: { name: String(name).slice(0, 60), visibility: "invite-only", ranking: "disabled", members: [...members.values()].map(({ ownerId: id, role }) => ({ ownerId: id, role })), shares: shares.filter((item) => item.visible).map(clone), encouragements: encouragements.filter((item) => item.targetId === viewerId).map(clone), challenges: challenges.map(clone), preferences: clone(preferences.get(viewerId) || { encouragementNotifications: false }) } }; },
  });
}
